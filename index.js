...
if (isMobileDevice()) {
    thumbnailPath = 'mobile thumbnails';
    sessionStorage['isMobileDevice'] = true;
}
...
function build_mediaClickListItems() {
    ...
    if (mediaClickListItems) {
        for (var i = 0; i < mediaClickListItems.length; i++) {
            ...
            var mediaCat = eval(unspace(mediaTitle).replace(/'/g, '').replace(/:/g, ''))['cat'];
            var mediaType = eval(unspace(mediaTitle).replace(/'/g, '').replace(/:/g, ''))['mediaType'];
            ...
            if (mediaType == 'movie') {
                ...
                <div class='listItemChoice searchItemChoice transport-button' onclick='transport("movie", "` + mediaTitle + `")'>
                    <div class='listItemTitle'>
                        <div class="listItemImageFrame" style="background-image: url('` + thumbnailPath + `/` + mediaTitle.replace(/\s/g, '-').replace(/'/g, '').replace(/:/g, '').toLowerCase() + `.jpg')"></div>
                        ` + mediaTitle + `
                    </div>
                ...
                mediaClickListItems[i].innerHTML = `
                <div class='listItemChoice searchItemChoice transport-button' onclick="expandTv('` + mediaTitle + `')">
                    <div class='listItemTitle'>
                        <div class="listItemImageFrame" style="background-image: url('` + thumbnailPath + `/` + mediaTitle.replace(/\s/g, '-').replace(/'/g, '').replace(/:/g, '').toLowerCase() + `.jpg')"></div>
                        ` + mediaTitle + `
                    </div>
    ...
    <div class='expand-screen-contents' id='tvScreenContents'>
        ...
        <div class='expand-panel purple-background' id='tvPanel' onscroll='tvExpandPanelScrolled()'>
            ...
            <div class='expand-panel-contents'>
                <div id='tvPanelNavbar'>
                    <div id='tvPanelNavbarContents' class='horizontal-scroll'></div>
                </div>
